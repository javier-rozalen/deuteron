...
parser = argparse.ArgumentParser(
    ...
    description="Computes the variance in the trained NN wave functions.",
...
parser.add_argument(
    "--show-plot",
    help="Whether to show the plot after execution or not (default: True)",
    default=True,
    choices=[True, False],
...
parser.add_argument(
    "--hidden-nodes",
    help="List of hidden nodes to use in the calculations (default: 20 30 40 60 80 100)",
...
parser.add_argument(
    "--hidden-nodes-plot",
    help="Number of hidden nodes to use for plot (default: 100)",
    default=100,
...
nhid_for_plot = args.hidden_nodes_plot
...
dict_dict_of_seeds = {arch: {} for arch, _ in zip(net_archs, range(len(net_archs)))}
for arch in net_archs:
    ...
    for nhid in tqdm(hidden_neurons):
        ...
        for model in list_of_trained_models:
            ...
            output_file = (
                f"{path_to_output_files}/wf_variance_nhid{nhid}" f"_ntest{n_test}.txt"
            )
            ...
            def test_loop(test_set, model):
                ...
                with torch.no_grad():
                    ...
                    norm_s_ann = integration.gl64_function(
                        norm_s2, test_a, test_b, x_i, w_i
                    )
                    norm_d_ann = integration.gl64_function(
                        norm_d2, test_a, test_b, x_i, w_i
                    )
                    ...
                    if sqrt_norm_s_ann > 0.0 and sqrt_norm_d_ann > 0.0:
                        for x in test_set:
                            if arch != "2sc":
                            ...
                            if (
                                math.isnan(pred_s) == False
                                and math.isnan(pred_d) == False
                            ):
                                psi_s_pred.append((pred_s / sqrt_norm_s_ann).item())
                                psi_d_pred.append((pred_d / sqrt_norm_d_ann).item())
                            else:
                                break
        ...
        with open(f"{path_to_output_files}/{output_file}", "r") as file:
            ...
            for line in file.readlines():
                if len(line.split(" ")) == 2:
                    ...
                    if len(all_psi_s) < len(seeds_of_filtered_runs) and len(
                        all_psi_d
                    ) < len(seeds_of_filtered_runs):
    ...
    with open(f"plot_data/wf_{arch}.txt", "w") as f:
        ...
        for psi_s, psi_d in zip(
            the_big_boy_s[nhid_for_plot], the_big_boy_d[nhid_for_plot]
        ):
    ...
    with open(f"plot_data/var_{arch}.txt", "w") as f:
        ...
        for nhid in hidden_neurons:
            nhid = 32 if arch == "2sd" and nhid == 30 else nhid
            for sigma_s, sigma_d in zip(
                the_big_boy_stdev_s[nhid], the_big_boy_stdev_d[nhid]
            ):
    ...
    axins.set_xticks(np.arange(0.0, 0.0401, 0.02))
    ...
    ax1.set_xlim(0, 4)
    # ax1.set_ylim(0.,1.25)
    ax1.set_xticks(np.arange(0.0, 4.001, 1.0))
    ...
    ax3.set_xticks(np.arange(0.0, 4.001, 1.0))
