...
############# IMPORTS #############
import torch, statistics, os, sys
...
n3lo = N3LO.N3LO(
    f"../../deuteron_data/q_uniform_{n_test}_10000/2d_vNN_J1.dat",
...
psi_target_s_norm = integrate.simpson(
    [i * j for i, j in zip(q_test2, psi_target_s2)], q_test
)
psi_target_d_norm = integrate.simpson(
    [i * j for i, j in zip(q_test2, psi_target_d2)], q_test
)
...
for key in list(dict_psi_s.keys()):
    if key.split("_")[0][2] == "d":
        ...
        wf_s_norm = integrate.simpson(
            [q * psi**2 for q, psi in zip(q_test2, wf_s)], q_test
        )
        wf_d_norm = integrate.simpson(
            [q * psi**2 for q, psi in zip(q_test2, wf_d)], q_test
        )
...
print("\nFIDELITIES")
print("----------------------------------------------------------------------")
print("FIDELITY VALUES OF MODEL 1sd")
print("--------------------------------")
print("Mean fidelities_s of 1sd: {:.5f}".format(statistics.mean(sd1_fidelities_s)))
print("Sigma(fidelities_s) of 1sd: {:.5f}".format(statistics.stdev(sd1_fidelities_s)))
print("Mean fidelities_d of 1sd: {:.5f}".format(statistics.mean(sd1_fidelities_d)))
print("Sigma(fidelities_d) of 1sd: {:.5f}".format(statistics.stdev(sd1_fidelities_d)))
print("\nFIDELITY VALUES OF MODEL 2sd")
print("--------------------------------")
print("Mean fidelities_s of 2sd: {:.5f}".format(statistics.mean(sd2_fidelities_s)))
print("Sigma(fidelities_s) of 2sd: {:.5f}".format(statistics.stdev(sd2_fidelities_s)))
print("Mean fidelities_d of 2sd: {:.5f}".format(statistics.mean(sd2_fidelities_d)))
print("Sigma(fidelities_d) of 2sd: {:.5f}".format(statistics.stdev(sd2_fidelities_d)))
...
for key in list(dict_psi_s.keys()):
    if key.split("_")[0][2] == "d":
        ...
        K_s_exact = (hbar**2 / mu) * (integrate.simpson(integrable_s_exact, q_test))
        K_d_exact = (hbar**2 / mu) * (integrate.simpson(integrable_d_exact, q_test))
