...
for arch, nhid, optim, actfun, lr, alpha, mom in product(net_archs, 
    ...
    if len(os.listdir()) > 0:
        ...
        print(f'{arch} nhid{nhid} optim{optim} ' \
              f'actfun{actfun} lr{lr} alpha{alpha} mu{mom}')
            
############# PRINTING #############
os.chdir(initial_dir)
print('MODELS TRAINED WITH "error_measure.py"')
print('------------------------------------')
for path in l_few_trained_models:
    print(f'Files at {path} only have ' \
          f'{len(os.listdir(path))}/{n_models} trained models.')
if len(l_few_trained_models) == 0:
    print(f'There are {n_models}/{n_models} trained models for the specified '\
...
for path in l_few_analyzed_models:    
    if type(path) == str:    
        print(f'File at {path} only has ' \
              f'{l_few_analyzed_models[l_few_analyzed_models.index(path)]}/' \
                  f'{n_models} analyzed models.')
if len(l_few_analyzed_models) == 0:
    print(f'There are {n_models}/{n_models} analyzed models for the specified'\
          ' hyperparameter combination.')
...
if len(l_few_good_runs) == 0:
    print(f'All models have {good_runs_min}/{good_runs_min} converged models '\
