...
def show_layers(model):
    ...
    for name, param in model.named_parameters():
        print(f"Layer: {name} | Size: {param.size()} | Values : " f"{param[:100]} \n")
...
def pretrain_loop(
    ...
    """
    optimizer.zero_grad()
    (loss, psi_s_pred, psi_d_pred, k_s, k_d) = loss_fn(
        model=model,
        arch=arch,
    ...
    return psi_s_pred, psi_d_pred, k_s, k_d, loss
...
def train_loop(
    ...
    optimizer.zero_grad()
    (loss, ann_s, ann_d, norm2, K, U, E, ks, kd, pd) = loss_fn(
        model=model,
        train_data=train_data,
        q_2=q_2,
        arch=arch,
