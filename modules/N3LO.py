...
class N3LO:
	...
	def __init__(self, filename, filename2):
		...
		for ik in range(N):
			for jk in range(N):
				(self.k[ik,jk], self.kp[ik,jk], _, _, self.vNN_S[ik,jk], 
     self.vNN_D[ik,jk], self.vNN_SD[ik,jk], self.vNN_DS[ik,jk]) = f[64*ik+jk,:]
	...
	def getOrbitalPotentials(self):
		return (torch.Tensor(self.vNN_S)*(self.hbar**3), 
          torch.Tensor(self.vNN_SD)*(self.hbar**3), 
          torch.Tensor(self.vNN_DS)*(self.hbar**3), 
          torch.Tensor(self.vNN_D)*(self.hbar**3))

	def getWavefunction(self):
		return (torch.Tensor(self.ksd).unsqueeze(1), 
          torch.Tensor(self.wfs).unsqueeze(1), 
          torch.Tensor(self.wfd).unsqueeze(1))
