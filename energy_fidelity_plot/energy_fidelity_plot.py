...
for arch in net_archs:
    ...
    with open(f'plotting_data/master_plot_data_{arch}.txt', 
              'w') as global_file:
        global_file.close()
    ...
    for Nhid in hidden_neurons:
        ...
        path_to_error_files = f'../error_analysis/error_data/' \
        ...
        list_of_error_files = [file for file in all_files if file == 
                               'means_and_errors.txt']
        ...
        E_osc_shade_top = [energies_joint[i]+abs(E_top_errors_joint[i]) 
                           for i in range(len(energies_joint))]
        E_osc_shade_bot = [energies_joint[i]-abs(E_bot_errors_joint[i]) 
                           for i in range(len(energies_joint))]
        E_stoch_shade_top = [energies_joint[i]+abs(mean_E_errors[i]) 
                             for i in range(len(energies_joint))]
        E_stoch_shade_bot = [energies_joint[i]-abs(mean_E_errors[i]) 
                             for i in range(len(energies_joint))]
        
        ks_osc_shade_top = [ks_joint[i]+abs(ks_top_errors_joint[i]) 
                            for i in range(len(ks_joint))]
        ks_osc_shade_bot = [ks_joint[i]-abs(ks_bot_errors_joint[i]) 
                            for i in range(len(ks_joint))]
        ks_stoch_shade_top = [ks_joint[i]+abs(mean_ks_errors[i]) 
                              for i in range(len(ks_joint))]
        ks_stoch_shade_bot = [ks_joint[i]-abs(mean_ks_errors[i]) 
                              for i in range(len(ks_joint))]
        
        kd_osc_shade_top = [kd_joint[i]+abs(kd_top_errors_joint[i]) 
                            for i in range(len(kd_joint))]
        kd_osc_shade_bot = [kd_joint[i]-abs(kd_bot_errors_joint[i]) 
                            for i in range(len(kd_joint))]
        kd_stoch_shade_top = [kd_joint[i]+abs(mean_kd_errors[i]) 
                              for i in range(len(kd_joint))]
        kd_stoch_shade_bot = [kd_joint[i]-abs(mean_kd_errors[i]) 
                              for i in range(len(kd_joint))]
            
        # We save the data in a global file.
        with open(f'plotting_data/master_plot_data_{arch}.txt', 
                  'a') as global_file:
            for (E, E_osc_bot, E_osc_top, E_stoch_bot, E_stoch_top, Fs, 
                 Fs_osc_bot, Fs_osc_top, Fs_stoch_bot, Fs_stoch_top, Fd, 
                 Fd_osc_bot, Fd_osc_top, Fd_stoch_bot, 
                 Fd_stoch_top) in zip(energies_joint, E_osc_shade_bot, 
                                      E_osc_shade_top,E_stoch_shade_bot,
                                      E_stoch_shade_top, ks_joint, 
                                      ks_osc_shade_bot, ks_osc_shade_top, 
                                      ks_stoch_shade_bot, ks_stoch_shade_top,
                                      kd_joint, kd_osc_shade_bot, 
                                      kd_osc_shade_top, kd_stoch_shade_bot,
                                      kd_stoch_shade_top):
                global_file.write(f'{E} {E_osc_bot} {E_osc_top} {E_stoch_bot}'\
        ...
        print(f'Error data of model {arch} nhid{Nhid} appended to ' \
...
if True:
    ...
    data_list = ['E', 'E_osc_bot', 'E_osc_top', 'E_stoch_bot', 'E_stoch_top',
                 'Fs', 'Fs_osc_bot', 'Fs_osc_top', 'Fs_stoch_bot', 
                 'Fs_stoch_top', 'Fd', 'Fd_osc_bot', 'Fd_osc_top', 
                 'Fd_stoch_bot', 'Fd_stoch_top']
...
for arch in net_archs: 
    ...
    axE.axhline(y=-2.2267, color="red", linestyle="--", 
                label='Exact' if index == 0 else '')
    axE.annotate(text=letters_E[index], 
                 xy=(0.85, 0.85),
                 xycoords='axes fraction',
                 fontsize=17)
    if show_osc_error:
        axE.fill_between(x_axis, dict_E[f'{arch}_E_osc_bot'], 
                         dict_E[f'{arch}_E_osc_top'], edgecolor='#089FFF', 
                         facecolor='#089FFF', alpha=0.3)
    if show_stoch_error:
        axE.fill_between(x_axis, dict_E[f'{arch}_E_stoch_bot'], 
                         dict_E[f'{arch}_E_stoch_top'], edgecolor='#089FFF', 
    ...
    axF.axhline(y=1., color="red", linestyle="--")
    axF.annotate(text=letters_F[index], 
                 xy=(0.85, 0.12),
                 xycoords='axes fraction',
                 fontsize=17)
    if show_osc_error:
        axF.fill_between(x_axis, dict_F[f'{arch}_Fs_osc_bot'], 
                         dict_F[f'{arch}_Fs_osc_top'], facecolor='purple', 
                         alpha=0.3)
        axF.fill_between(x_axis, dict_F[f'{arch}_Fd_osc_bot'], 
                         dict_F[f'{arch}_Fd_osc_top'], 
                         facecolor=d_shades['osc'], alpha=d_shades['alpha'])
    if show_stoch_error:
        axF.fill_between(x_axis, dict_F[f'{arch}_Fs_stoch_bot'], 
                         dict_F[f'{arch}_Fs_stoch_top'], facecolor='purple', 
                         alpha=0.5)
        axF.fill_between(x_axis, dict_F[f'{arch}_Fd_stoch_bot'], 
                         dict_F[f'{arch}_Fd_stoch_top'], 
                         facecolor=d_shades['stoch'], alpha=d_shades['alpha'])
    axF.plot(x_axis, dict_F[f'{arch}_Fs'], color='purple', 
             label='S-state' if index == 0 else '')
    axF.plot(x_axis, dict_F[f'{arch}_Fd'], color='green', 
             label='D-state' if index == 0 else '')

fig.legend(loc='lower center', bbox_to_anchor=(0.5, -0.14), ncol=5, 
